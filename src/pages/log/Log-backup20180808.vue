  ...
  <div class="page">
    ...
    <div class="result-panel">
      <div class="result-panel-toolbar">
        <h3 class="result-panel-title">日志 <span v-if="logRecord.length > 0">共{{logRecord.length}}条</span></h3>
      ...
      <el-collapse v-if="logRecord.length > 0" v-model="activeLogIndexs" class="result-pannel-inner" :accordion="toggleLogItemVisibleClass.type === 'fa-window-minimize'">
        <el-collapse-item v-for="(log, index) in logRecord" :key="index" :name="index" @hover="addActiveNamesAction(index)">
          <template slot="title">
            <span class="text-label text-label-blue">{{log.dateTime}}</span>
            <span class="text-label">{{log.connector}}（{{interfaceCN[log.connector]}}）</span>
          </template>
          <el-row :gutter="20">
            <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    //     arrNew[i].dateTime = arr[i].app.request.dateTime
    //     arrNew[i].connector = arr[i].app.request.connector
    ...
    jsonFormatterAppRes (arr) {
      ...
      for (var i = 0; i < arr.length; i++) {
        ...
        arrNew[i].connector = arr[i].connector
    ...
    getLogData (params) {
      ...
      // }
      params = {
        minDate: '2018-08-03 10:30:26',
        maxDate: '2018-08-03 13:37:26',
        call: '',
        mobileNo: '',
        uuid: ''
      }
      getLogDataReg(params).then(res => {
        ...
        if (data) {
          ...
          this.logRecord = data
          if (this.logRecord.length > 0) {
            this.logRecord = this.jsonFormatterAppReq(this.logRecord.appList)
            this.logRecord = this.jsonFormatterAppRes(this.logRecord.appList)
            this.logRecord = this.jsonFormatterPartnerReq(this.logRecord.partnerList)
            this.logRecord = this.jsonFormatterPartnerRes(this.logRecord.partnerList)
          }
        }
      }).catch(err => {
        console.log(err)
